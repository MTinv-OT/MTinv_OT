...
class OptimizerConfig:
    ...
    def create_sinkhorn_loss(self,
                            ...
                            blur: float = 0.08,
                            scaling: float = 0.9,
                            reach: Optional[float] = None,  # None=平衡(默认); >0=非平衡
                            debias: bool = True,
        ...
        参数:
            ...
            reach: 非平衡传输的松弛距离。None=平衡OT(强约束); 值(如1.0)=非平衡OT(软约束)
            debias: 是否去偏
        ...
        sinkhorn_loss = SamplesLoss(
            ...
            blur=blur,
            scaling=scaling,
            reach=reach,     # <--- [修改2] 必须显式将参数传给 GeomLoss
            debias=debias,
            backend=backend
        )
        ...
        print(f"  - reach: {reach} ({'非平衡/软约束' if reach else '平衡/强约束'})") # <--- [修改3] 打印状态以便确认
