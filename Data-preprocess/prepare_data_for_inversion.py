...
# Script uses wide range 100Hz~0.1Hz; will auto-truncate.
target_freqs = np.logspace(np.log10(10000), np.log10(0.1), num=40) 

DEFAULT_STRIKE = 45.0  # Prior value; will be auto-calculated from data if mtpy available
# ===========================================
...
def read_custom_edi(file_path):
    ...
    # 2. Assemble data
    # WinGLink often uses >FREQ for frequency
    ...
    if not has_err:
        Z_err = None  # No error; will use 10% floor later
...
# 3. Auto-compute strike (use mtpy if available)
...
# 4. Rotate, interpolate and pack data
data_list = []
error_list = []
station_locs = []
...
for mt in mt_objects:
    ...
    # Note: freq may be ascending/descending; interp1d usually handles both
    # 最好传入 log10(freq)
    log_orig_f = np.log10(orig_freq)
    log_target_f = np.log10(target_freqs)
    
    # Simple range check to avoid all-NaN interpolation
